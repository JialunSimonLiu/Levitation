...
double timefunc(double time) {
    return time/CONTIME;//Changing potential 
    //return TIMEXPRESS;//Fixed potential; time in units of Myr
}
...
//    void getsimplestartperiods(GalaxyPotential &pH, GalaxyPotential &pD, periods[2], double maxtime);
    ...
    void fourier(GalaxyPotential &pH1, GalaxyPotential &pH2, ostringstream &ostr1, ostringstream &ostr2, ostringstream &ostr3);
...
void particle::fourier(GalaxyPotential &pH1, GalaxyPotential &pH2, ostringstream &ostr1, ostringstream &ostr2, ostringstream &ostr3){    
    ...
    //ostr22 is temporarily used to find the position of the maximum values in the array 
    for (int i = 0; i < M; i++) {
        ti=1.0*delta*i;
...
void Dotest1() {
    ...
    //fix the potential
    //fourier analysis
    pp.fourier(pH,pD,ostr1,ostr2,ostr3);
